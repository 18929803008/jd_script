            ...
            if (safeGet(data)) {
              ...
              if (data['ret'] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  jxfactory_ShareCode.push(token)
                }
              } else {
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map(item => {
                  if (item.taskType === 14) {
                    ...
                    let token = taskToken
                    ddfactory_ShareCode.push(token)
                  }
                })
              }
            }
...
// ------------【常驻活动】------------
let bean_ShareCode = [] // 种豆得豆
let farm_ShareCode = [] // 东东农场
let pet_ShareCode = [] // 东东萌宠
let jxfactory_ShareCode = [] // 京喜工厂
let ddfactory_ShareCode = [] // 东东工厂
// ------------【临时活动】------------
let jdSgmh = [] // 闪购盲盒
...
async function getJdZZ() {
  ...
  function taskZZUrl(functionId, body = {}) {
    return {
      url: `${JDZZ_API_HOST}?functionId=${functionId}&body=${escape(JSON.stringify(body))}&appid=swat_miniprogram&osVersion=5.0.0&clientVersion=3.1.3&fromType=wxapp&timestamp=${new Date().getTime() + new Date().getTimezoneOffset() * 60 * 1000 + 8 * 60 * 60 * 1000}`,
      headers: {
        "Accept": "*/*",
        "Accept-Encoding": "gzip, deflate, br",
        "Accept-Language": "zh-cn",
...
async function getJDCash() {
  ...
  function taskCashUrl(functionId, body = {}) {
    return {
      url: `${JDCASH_API_HOST}?functionId=${functionId}&body=${escape(
        JSON.stringify(body)
...
function showFormatMsg() {
  ...
  console.log(`/submit_activity_codes bean ${bean_ShareCode.join('&')}\n`)
  console.log(`/submit_activity_codes farm ${farm_ShareCode.join('&')}\n`)
  console.log(`/submit_activity_codes pet ${pet_ShareCode.join('&')}\n`)
  console.log(`/submit_activity_codes jxfactory ${jxfactory_ShareCode.join('&')}\n`)
  console.log(`/submit_activity_codes ddfactory ${ddfactory_ShareCode.join('&')}\n`)
